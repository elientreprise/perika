...
use App\Dto\Response\Timesheet\TimesheetValidatedResponse;
...
readonly class TimesheetValidProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): mixed
    {
        ...
        if (!$this->security->isGranted(PermissionEnum::CAN_VALID_TIMESHEET->value, $data)) {
            throw new AccessDeniedHttpException($this->getVoterReason(TimesheetVoter::class));
        }

        $data->setStatus(TimesheetStatusEnum::VALID);
        $timesheet = $this->persistProcessor->process($data, $operation, $uriVariables, $context);

        return new JsonResponse(new TimesheetValidatedResponse(
